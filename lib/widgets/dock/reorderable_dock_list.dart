...
class _ReorderableDockListState extends State<ReorderableDockList>
  ...
  double _getItemScale(int index) {
    if (_draggedIndex == null || _targetIndex == null) {
      ...
      if (_hoveredIndex != null) {
        ...
        if (distance <= 1) {
          // Affects current and adjacent items
  ...
  double _getSpacing(int index) {
    ...
    final expandedSpacing =
        normalSpacing * 2.5; // Reduced from 10x to 2.5x for smoother feel
      ...
      if (_dragOffset != null) {
        ...
        if (targetRenderBox != null) {
          final targetCenter = targetRenderBox.localToGlobal(
            Offset(targetRenderBox.size.width / 2,
          ...
          proximityFactor =
              (1.0 - (distance / (widget.itemWidth * 0.5))).clamp(0.0, 1.0);
        }
      }

      final curvedValue = Curves.easeOutCubic.transform(_spaceController.value);
      return normalSpacing +
          (expandedSpacing - normalSpacing) * curvedValue * proximityFactor;
    }
  ...
  double _getItemOffset(int index) {
    ...
    if (movingRight) {
      if (index > _draggedIndex! && index <= _targetIndex!) {
        ...
        offset -= (widget.itemWidth + widget.itemSpacing) *
            Curves.easeOutBack.transform(_spaceController.value);
      }
    } else {
      if (index < _draggedIndex! && index >= _targetIndex!) {
      ...
      child: Draggable<int>(
        ...
        onDragEnd: (details) {
          ...
          if (_targetIndex != null && _dragOffset != null) {
            final targetBox = _itemKeys[_targetIndex!]
                .currentContext
          ...
          setState(() {
            ...
            if (_draggedIndex != null) {
              ...
              for (int i = 0; i < _itemKeys.length; i++) {
                ...
                final itemBox = _itemKeys[i].currentContext?.findRenderObject()
                    as RenderBox?;
                if (itemBox != null) {
          ...
          builder: (context, offset, child) {
            Widget result = Transform.translate(
              ...
              child: AnimatedScale(
                ...
                duration:
                    const Duration(milliseconds: 800), // Increased from 200ms
                curve: Curves.easeOutCubic,
                child: child,
              ),
            );
            ...
            if (_isReordering && _itemPositions.containsKey(index)) {
              ...
              final box = _itemKeys[index].currentContext?.findRenderObject()
                  as RenderBox?;
              if (box != null) {
                ...
                result = TweenAnimationBuilder<Offset>(
                  ...
                  duration: const Duration(
                      milliseconds: 1200), // Increased from 600ms
                  curve: Curves.easeOutCubic,
                  tween: Tween<Offset>(
                    begin: -diffOffset,
                    end: Offset.zero,
      ...
      if (i > 0) {
        items.add(
          TweenAnimationBuilder<double>(
            duration:
                const Duration(milliseconds: 1000), // Increased from 300ms
            curve: Curves.easeOutCubic,
            tween: Tween<double>(
